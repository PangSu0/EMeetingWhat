    ...
    protected class MainView extends View {
        ...
        public void drawPresent(Canvas canvas) {
            ...
            for( int i=0; i < mPeopleMax; i++) {
                ...
                // 사각형 영역 안에 텍스트 출력
                String personNumber = "" + (i + 1);
                drawText(canvas, rtPresentPart, personNumber);
            }
        ...
        public void createGame(Canvas canvas) {
            ...
            for(int i=0; i < mPeopleMax*2; i++) {
                ...
                }while( checkCross(ptHBar.x, ptHBar.y) );
                mArHBar.add(ptHBar);
            }
        }
        public boolean checkCross(int positionX, int positionY) {
            for(int i = 0 ; i < mArHBar.size() ; i++)
            {
                if((mArHBar.get(i).y - positionY >= -15 && mArHBar.get(i).y - positionY <= 15)    //Y좌표 체크
                        && (mArHBar.get(i).x - 1 == positionX || mArHBar.get(i).x == positionX || mArHBar.get(i).x + 1 == positionX)) { //X좌표 체크
                    return true;
                }
            }
            return false;
